...
| Argo CD root app `Unknown / Unknown` with `namespace 'argocd' do not match any of the allowed destinations` | Platform AppProject's `destinations` doesn't include the namespace Argo CD itself runs in | Allow `argocd` (and `gatekeeper-system`) in `argocd/projects/platform.yaml` |
| KPS sync fails with `namespace kube-system is not permitted` | KPS creates scrape Services in kube-system for coreDns + kubelet; platform AppProject's destinations doesn't allow kube-system | Add `kube-system` to **platform** AppProject only (never to tenant projects) |
| KPS PreSync hooks fail with `namespaces "monitoring" not found` | Helm `CreateNamespace=true` runs in the regular sync wave, AFTER PreSync hooks. Hooks can't find the namespace | Pre-create platform namespaces at sync-wave -3 via `argocd/manifests/platform-namespaces/` |
| cert-manager sync fails with `namespace kube-system is not permitted` | Chart writes leader-election Role/RoleBinding into kube-system by default | Set `global.leaderElection.namespace: cert-manager` in chart values |
| `ServiceMonitor` CRD not found during cert-manager / ingress-nginx / loki / grafana sync | KPS (which provides the CRD) lands in the same or later wave | Promote KPS to sync-wave **-2** so its CRDs precede everything that uses them |
| Gatekeeper Constraints fail with `K8sRequireResourceLimits CRD not installed` | Constraint CRDs are dynamically registered by Gatekeeper from ConstraintTemplates; same-wave application races | Put Templates at sync-wave 0, Constraints at sync-wave 1 (per-resource annotation) |
| `Quota 'N2D_CPUS' / 'CPUS_ALL_REGIONS' / 'SSD_TOTAL_GB' exceeded` | Fresh project default quotas are tight (16 vCPU N2D, 32 vCPU all-regions, 500GB SSD per region) | Request quota increases in GCP console; auto-approval typically < 1 hour |
| ingress-nginx CrashLoopBackOff with `read-only file system` on various paths | `readOnlyRootFilesystem: true` conflicts with the controller's design (rewrites nginx.conf and other files on every reload). The chart doesn't ship a clean read-only-root mode for 1.11.x | Set `readOnlyRootFilesystem: false` on ingress-nginx-controller specifically; keep other hardening (non-root, dropped caps). See compliance-notes "ingress-nginx exception" |
