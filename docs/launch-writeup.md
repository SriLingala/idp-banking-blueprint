...
- **Production `environments/prod`.** Identical to dev modulo sizing, quotas, and `enable_backup = true`. Adding it is mechanical; doing it badly would set a precedent. v1.1.

## How I'd extend this if I owned it

In rough order of leverage:

1. **`environments/prod`** + the matching platform-stack split (`environments/dev-platform/` and `environments/prod-platform/`). The kubernetes/helm provider bootstrap problem is real; the cleanest answer is a sibling stack that reads cluster outputs via `terraform_remote_state`.
...
5. **Disaster-recovery drill runbook.** Backup for GKE is configured; the drill is the missing piece.
6. **Federation Sentinel policies** to be shared across multiple Terraform Cloud workspaces (not just the cluster workspace), so an out-of-region resource provisioned by a sibling team is also caught.
...
4. **Day 3** — Walk `policies/`. Run `sentinel test` and `opa test` (when those tests exist; see follow-ups).
